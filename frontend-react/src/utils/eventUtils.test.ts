...
describe("deriveStatus", () => {
  ...
  it("returns upcoming when start is future even with a future end date", () => {
    expect(deriveStatus("2099-01-01T00:00:00Z", "2099-01-05T00:00:00Z")).toBe(
      "upcoming",
    );
  });

  it("returns live when start is past and end date is in the future", () => {
    expect(deriveStatus("2020-01-01T00:00:00Z", "2099-12-31T00:00:00Z")).toBe(
      "live",
    );
  });

  it("returns completed when both start and end are in the past", () => {
    expect(deriveStatus("2020-01-01T00:00:00Z", "2020-01-05T00:00:00Z")).toBe(
      "completed",
    );
  });
...
describe("sortedRounds", () => {
  it("sorts rounds in bracket progression order", () => {
    ...
    expect(sortedRounds(input)).toEqual([
      "winners_r1",
      "winners_finals",
      "elim_r1",
      "grand_finals",
