...
export const countryFlag = (code: string) =>
  COUNTRY_FLAGS[code?.toUpperCase()] ?? "🌐";
...
const ROUND_ORDER: Record<string, number> = {
  ...
  quarterfinal: 40,
  semifinal: 60,
  third_place_match: 85,
  opening_match: 10,
  winners_match: 20,
  decider_match: 30,
  elimination_match: 40,
  round_1: 10,
...
export const eventDisplayName = (slug: string, name: string) =>
  CW_SLUG_NAMES[slug] ?? name;
...
export const formatTournamentFormat = (f?: string | null): string =>
  FORMAT_LABELS[f ?? ""] ?? (f || "—");

export const formatRound = (r: string) =>
  ROUND_LABELS[r] ??
  r.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase());
export const roundOrder = (r: string) => ROUND_ORDER[r] ?? 99;
export const bracketSection = (
  r: string,
): "winners" | "elimination" | "grand_finals" =>
  r === "grand_finals"
    ? "grand_finals"
    : r.startsWith("winners")
      ? "winners"
      : "elimination";
