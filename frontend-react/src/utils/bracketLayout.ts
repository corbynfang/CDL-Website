...
import { formatRound } from "./eventUtils";
...
function colLabels(nodes: MatchNode[]) {
  ...
  return [...map.entries()]
    .sort(([a], [b]) => a - b)
    .map(([col, label]) => ({ col, x: col * COL_STRIDE, label }));
}
...
const EWC_DECIDER_Y = Math.round(
  (EWC_OPEN_Y + CARD_H / 2 + EWC_OPEN_Y + EWC_OPEN_GAP + CARD_H / 2) / 2 -
    CARD_H / 2,
);
...
function layoutEWC(data: BracketData): BracketLayout {
  ...
  for (let gi = 0; gi < GROUPS.length; gi++) {
    ...
    if (wMatches.length > 0) {
      ...
      nodes.push(wNode);
      nodeById.set(wMatches[0].id, wNode);
    }
    ...
    if (eMatches.length > 0) {
      ...
      nodes.push(eNode);
      nodeById.set(eMatches[0].id, eNode);
    }
    ...
    if (dMatches.length > 0) {
      dNode = mkNode(
        ...
        `${pre}decider_match`,
        2,
        bandTop + EWC_DECIDER_Y,
        "group",
      );
      dNode.roundLabel = formatRound("decider_match");
      nodes.push(dNode);
      nodeById.set(dMatches[0].id, dNode);
    }
  ...
  for (let gi = 0; gi < GROUPS.length; gi++) {
    ...
    const wNode =
      nodes.find((n) => n.roundKey === `${pre}winners_match`) ?? null;
    const dNode =
      nodes.find((n) => n.roundKey === `${pre}decider_match`) ?? null;
  ...
  if (sfNodes.length >= 2) {
    ...
    if (gfMatches.length > 0) {
      ...
      nodes.push(gfNode);
      nodeById.set(gfMatches[0].id, gfNode);
    }
    if (tpMatches.length > 0) {
      ...
      nodes.push(tpNode);
      nodeById.set(tpMatches[0].id, tpNode);
    }
...
function layoutCDLDoubleElim(data: BracketData): BracketLayout {
  ...
  function lrY(pos: number, count: number): number {
    ...
    return (
      LR_TOP_OFFSET + (pos - 1) * groupH + Math.round(groupH / 2 - CARD_H / 2)
    );
  }
  ...
  for (const [roundKey, matches] of roundsByKey) {
    ...
    for (const m of matches) {
      ...
      if (isGF) {
        ...
        y = Math.round(
          (LR_TOP_OFFSET + 0 + WR_TOP + 3 * ROW_H) / 2 - CARD_H / 2,
        );
      } else if (isWR) {
      ...
      const section: MatchNode["section"] = isGF
        ? "final"
        : isWR
          ? "winners"
          : "losers";
