...
// ─── Subcomponent: scoreboard for one map ────────────────────────────────────
...
const ModeIcon = ({ mode }: { mode: string }) => {
  ...
  return <span className="font-mono text-[10px] text-[#737373]">{icons[mode] ?? mode.slice(0, 3).toUpperCase()}</span>;
};
...
const Scoreboard = ({ map, team1Name, team2Name, team1ID }: ScoreboardProps) => {
  ...
  const PlayerRow = ({ p, highlight }: { p: PlayerStat; highlight: boolean }) => (
    <tr className={`border-b border-[#111111] transition-colors ${highlight ? "bg-[#0f0f0f]" : ""}`}>
      ...
      <td className={`px-2 py-2 text-xs font-bold font-mono text-right ${kdColor(p.kd_ratio)}`}>
        {p.kd_ratio.toFixed(2)}
      </td>
      <td className="px-2 py-2 text-xs font-mono text-right text-white">{p.kills}</td>
      <td className="px-2 py-2 text-xs font-mono text-right text-[#737373]">{p.deaths}</td>
  ...
  const teamTable = (players: PlayerStat[], teamName: string, score: number, won: boolean) => (
    <div className="flex-1 min-w-0">
      <div className={`flex items-center justify-between px-3 py-2 border-b border-[#1a1a1a] ${won ? "bg-white/[0.03]" : ""}`}>
        <span className={`font-grotesk font-bold text-xs ${won ? "text-white" : "text-[#737373]"}`}>
          {teamName}
          {won && <span className="ml-2 text-[10px] text-green-400 uppercase tracking-widest">W</span>}
        </span>
        <span className={`font-mono font-bold text-lg ${won ? "text-white" : "text-[#737373]"}`}>{score}</span>
      </div>
      <table className="w-full">
        <thead>
          <tr className="border-b border-[#1a1a1a]">
            <th className="px-3 py-1.5 text-left text-[#737373] text-[10px] uppercase tracking-widest font-medium">Player</th>
  ...
  return (
    <div className="border border-[#1a1a1a] overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-[#0a0a0a] border-b border-[#1a1a1a]">
        <span className="text-white font-grotesk font-bold text-sm">Map {map.map_number}</span>
        ...
        {map.duration_sec > 0 && (
          <>
            <span className="text-[#737373] text-xs">·</span>
            <span className="text-[#737373] text-xs">{formatDuration(map.duration_sec)}</span>
          </>
        )}
      </div>
...
const MatchDetail = () => {
  ...
  const { data, loading, error } = useApi<MatchDetailResponse>(`/api/v1/matches/${id}`);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="bg-[#111111] border border-[#1a1a1a] p-6 mb-8">
        <div className="flex items-center justify-between gap-4">
          ...
          >
            {team1Logo ? (
              <img src={team1Logo} alt={match.team1_name} className="w-20 h-20 object-contain opacity-90 group-hover:opacity-100 transition-opacity" />
            ) : (
            ...
            <div className="text-center">
              <p className={`font-grotesk font-bold text-sm ${team1Won ? "text-white" : "text-[#737373]"}`}>
                {match.team1_name}
              </p>
            </div>
          </Link>
          ...
          <div className="text-center flex-shrink-0 px-6">
            <div className="flex items-center gap-4">
              <span className={`font-mono font-black text-5xl ${team1Won ? "text-white" : "text-[#404040]"}`}>
                {match.team1_score}
              </span>
              <span className="text-[#404040] font-mono text-3xl">—</span>
              <span className={`font-mono font-black text-5xl ${team2Won ? "text-white" : "text-[#404040]"}`}>
                {match.team2_score}
              </span>
            </div>
            <p className="text-[#737373] text-xs uppercase tracking-widest mt-2">{match.format}</p>
          ...
          >
            {team2Logo ? (
              <img src={team2Logo} alt={match.team2_name} className="w-20 h-20 object-contain opacity-90 group-hover:opacity-100 transition-opacity" />
            ) : (
              <div className="w-20 h-20 bg-[#1a1a1a] flex items-center justify-center text-sm font-bold text-[#737373] font-mono">
                {match.team2_abbr}
              </div>
            ...
            <div className="text-center">
              <p className={`font-grotesk font-bold text-sm ${team2Won ? "text-white" : "text-[#737373]"}`}>
                {match.team2_name}
              </p>
            </div>
          </Link>
        ...
        <div className="space-y-4">
          {playedMaps.length > 0 ? (
            ...
            <div className="border border-[#1a1a1a] p-12 text-center">
              <p className="text-[#737373] text-sm">Map stats not available for this match</p>
            </div>
          )}
        </div>
      </div>
