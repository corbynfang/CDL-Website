...
const Home = () => {
  ...
  const { data: teamsData } = useApi<Team[]>("/api/v1/teams", { cacheTtl: 5 * 60 * 1000 });
  const { data: eventsData } = useApi<Tournament[]>("/api/v1/tournaments", { cacheTtl: 5 * 60 * 1000 });
  ...
  const { data: playersData, loading: playersLoading } = useApi<PaginatedResponse<Player>>(
    playerSearchUrl,
  ...
  const results = useMemo((): SearchResult[] => {
    ...
    const playerResults: SearchResult[] = (playersData?.data ?? [])
      .slice(0, 5)
      .map(p => ({ type: "Player", id: p.id, label: p.gamertag, href: `/players/${p.id}` }));

    const teamResults: SearchResult[] = (teamsData ?? [])
      .filter(t => t.name !== "Unaffiliated" && t.name.toLowerCase().includes(q))
      .slice(0, 3)
      .map(t => ({ type: "Team", id: t.id, label: t.name, href: `/teams/${t.id}` }));

    const eventResults: SearchResult[] = (eventsData ?? [])
      .filter(e => !isHidden(e.tournament_type) && e.name.toLowerCase().includes(q))
      .slice(0, 3)
      .map(e => ({ type: "Event", id: e.id, label: e.name, href: `/events/${e.slug}` }));
  ...
  const featuredEvents = useMemo((): Tournament[] => {
    ...
    const matched = FEATURED_FRAGS
      .map(frags => eventsData.find(e => fragMatch(e.name, frags)))
          ...
          {isSearching && (
            <div className="absolute top-full left-0 right-0 mt-px bg-[#111111] border border-[#2a2a2a] z-20 shadow-lg max-h-[60vh] overflow-y-auto">
              {isLoading && results.length === 0 && (
                <div className="px-5 py-3 text-[#737373] text-sm">Searching…</div>
              )}
              {!isLoading && results.length === 0 && (
              ...
              {results.map((r, i) => (
                ...
                >
                  ...
                  <span className="text-[#737373] text-xs uppercase tracking-widest ml-3 shrink-0">{r.type}</span>
                </Link>
              ))}
            </div>
          )}
