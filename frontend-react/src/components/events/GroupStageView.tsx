...
function EWCGroupSection({
	letter,
	groupPrefix,
	roundKeys,
	groupStage,
}: EWCGroupSectionProps) {
	// Sort by canonical round order using the suffix (strips group prefix, then uses ROUND_ORDER weights)
	const sorted = sortedRounds(
		roundKeys.map((k) => k.slice(groupPrefix.length)),
