...
describe("EventFilters", () => {
	...
	it("shows all game options", () => {
		...
		expect(
			screen.getByRole("option", { name: "Black Ops 6" }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("option", { name: "Modern Warfare III" }),
		).toBeInTheDocument();
	});
	...
	it("shows all type options", () => {
		...
		expect(
			screen.getByRole("option", { name: "Championship" }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("option", { name: "Qualifier" }),
		).toBeInTheDocument();
	});
	...
	it("reflects current filter values in the selects", () => {
		render(
			<EventFilters
				filters={{ game: "MW3", type: "qualifier", status: "completed" }}
				onChange={vi.fn()}
			/>,
		);
		const [gameSelect, typeSelect, statusSelect] = screen.getAllByRole(
			"combobox",
