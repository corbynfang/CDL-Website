...
describe("BracketControls", () => {
	...
	it("renders a button for each round in sorted order", () => {
		render(
			<BracketControls
				rounds={["grand_finals", "winners_r1"]}
				active={null}
				onSelect={vi.fn()}
			/>,
		);
		expect(
			screen.getByRole("button", { name: /winners round 1/i }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("button", { name: /grand finals/i }),
		).toBeInTheDocument();
	});
	...
	it("calls onSelect with the round key when a round button is clicked", async () => {
		...
		render(
			<BracketControls
				rounds={["winners_r1", "grand_finals"]}
				active={null}
				onSelect={onSelect}
			/>,
		);
	...
	it("formats unknown round keys into human-readable labels", () => {
		...
		expect(
			screen.getByRole("button", { name: /custom round/i }),
		).toBeInTheDocument();
	});
	...
	it("renders all CDL bracket round types", () => {
		...
		render(
			<BracketControls rounds={rounds} active={null} onSelect={vi.fn()} />,
		);
		expect(
			screen.getByRole("button", { name: /winners round 1/i }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("button", { name: /winners round 2/i }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("button", { name: /grand finals/i }),
		).toBeInTheDocument();
