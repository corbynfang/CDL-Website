		...
		return (
			<div className="space-y-2 animate-pulse">
				{Array.from({ length: 8 }).map((_, i) => (
					<div
						key={i}
						className="h-14 rounded border border-zinc-800 bg-zinc-900/60"
					/>
				))}
			</div>
		);
		...
		<div className="border border-[#1a1a1a] divide-y divide-[#1a1a1a]">
			...
			{sorted.map((team, idx) => {
				...
				return (
					...
					>
						<span className="col-span-1 text-sm text-zinc-600 tabular-nums">
							{placement}
						</span>
						...
						<span className="col-span-2 text-right text-sm text-zinc-400 tabular-nums">
							{team.matches_won ?? "—"}
						</span>
						<span className="col-span-2 text-right text-sm text-zinc-600 tabular-nums">
							{team.matches_lost ?? "—"}
						</span>
					</Link>
				);
