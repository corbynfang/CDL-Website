...
export default function BracketControls({
  ...
  const pill = 'text-[10px] uppercase tracking-widest px-3 py-1.5 border transition-colors'
  const active_cls   = 'border-white text-white'
  const inactive_cls = 'border-[#1a1a1a] text-zinc-600 hover:text-zinc-400 hover:border-[#2a2a2a]'
  const ctrl_cls     = 'border-[#1a1a1a] text-zinc-600 hover:text-zinc-300 hover:border-[#2a2a2a] disabled:opacity-30 disabled:cursor-not-allowed'

  return (
    <div className="flex flex-wrap items-center gap-2 justify-between">
      {/* Round filter */}
      <div className="flex flex-wrap gap-2">
        <button onClick={() => onSelect(null)} className={`${pill} ${active === null ? active_cls : inactive_cls}`}>
          All Rounds
        </button>
        {sorted.map(r => (
          <button key={r} onClick={() => onSelect(r)} className={`${pill} ${active === r ? active_cls : inactive_cls}`}>
            {formatRound(r)}
          </button>
        ))}
      </div>
