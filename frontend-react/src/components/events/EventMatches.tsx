...
function MatchRow({ match }: { match: Match }) {
  ...
  function TeamCell({ name, abbr, won }: { name?: string; abbr?: string; won: boolean }) {
    ...
    return (
      <div className={`flex items-center gap-2 ${won ? '' : 'opacity-50'}`}>
        ...
        <span className={`text-sm ${won ? 'text-white font-semibold' : 'text-zinc-400'} hidden sm:block`}>
          {name ?? '—'}
        </span>
        <span className={`text-sm ${won ? 'text-white font-semibold' : 'text-zinc-400'} sm:hidden`}>
          {abbr ?? '?'}
        </span>
      </div>
    )
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-sm font-bold tabular-nums flex-shrink-0">
        <span className={t1Won ? 'text-white' : 'text-zinc-600'}>{match.team1_score}</span>
        <span className="text-zinc-700">–</span>
        <span className={t2Won ? 'text-white' : 'text-zinc-600'}>{match.team2_score}</span>
      </div>
