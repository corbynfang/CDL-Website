...
describe("EventBracket", () => {
	it("shows BracketSkeleton while loading", () => {
		const { container } = wrap(
			<EventBracket data={null} loading={true} error={null} />,
		);
	...
	it("renders bracket content when data is provided", () => {
		wrap(
			<EventBracket data={sampleBracketData} loading={false} error={null} />,
		);
	...
	it("renders BracketControls when there are multiple rounds", () => {
		...
		expect(
			screen.getByRole("button", { name: /all rounds/i }),
		).toBeInTheDocument();
	});
	...
	it("does not render BracketControls when there is only one round", () => {
		const singleRound = {
			...sampleBracketData,
			bracket: { winners_r1: sampleBracketData.bracket.winners_r1 },
		};
		wrap(<EventBracket data={singleRound} loading={false} error={null} />);
		expect(
			screen.queryByRole("button", { name: /all rounds/i }),
	...
	it("standard CDL shows no tabs", () => {
		...
		expect(
			screen.queryByRole("button", { name: /bracket/i }),
		).not.toBeInTheDocument();
		expect(
			screen.queryByRole("button", { name: /group stage/i }),
		).not.toBeInTheDocument();
	});

	it("CDL major group bracket shows Bracket and Group Stage tabs", () => {
		wrap(
			<EventBracket data={cdlGroupBracketData} loading={false} error={null} />,
		);
		expect(
			screen.getByRole("button", { name: /^bracket$/i }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("button", { name: /^group stage$/i }),
		).toBeInTheDocument();
	});
