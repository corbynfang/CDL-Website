...
export default function EventBracket({ data, loading, error }: Props) {
  ...
  const [userTab,      setUserTab]      = useState<'bracket' | 'group_stage' | null>(null)
  ...
  if (error) {
    return <p className="text-center text-zinc-600 py-16 text-sm">Bracket data not available for this event.</p>
  }

  if (!data || data.total_matches === 0) {
    return <p className="text-center text-zinc-600 py-16 text-sm">No bracket matches have been played yet.</p>
  }

  const isEWC = data.event_format === 'ewc_group_stage_single_elim'

  // EWC: render the combined tree (group stage + playoff in one view, no tabs)
  if (isEWC) {
    return (
      <div
        ref={containerRef}
  ...
  // CDL formats: keep existing tab-based layout (BracketCanvas + GroupStageView)
    ...
    >
      {hasGroupStage && (
        <div className="flex gap-0 border-b border-[#1e1e1e]">
          <button onClick={() => setUserTab('bracket')} className={`${tabPill} ${activeTab === 'bracket' ? tabActive : tabInactive}`}>
            Bracket
          </button>
          <button onClick={() => setUserTab('group_stage')} className={`${tabPill} ${activeTab === 'group_stage' ? tabActive : tabInactive}`}>
            Group Stage
          </button>
        </div>
      )}
      ...
      {activeTab === 'bracket' && (
        <>
          ...
          <BracketCanvas data={data} activeRound={activeRound} zoom={zoom} flat={false} />
        </>
      )}

      {activeTab === 'group_stage' && data.group_stage && (
        <GroupStageView groupStage={data.group_stage} format={data.event_format ?? ''} />
      )}
    </div>
  )
}
