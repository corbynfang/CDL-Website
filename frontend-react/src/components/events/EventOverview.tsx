  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-2">
        <p className="text-xs uppercase tracking-widest text-zinc-600">Details</p>
        <div className="border border-[#1a1a1a] divide-y divide-[#1a1a1a]">
          {[
            ...
            event.location ? { label: 'Location', value: `${event.country ? countryFlag(event.country) + ' ' : ''}${event.location}` } : null,
            event.season?.name ? { label: 'Season', value: event.season.name } : null,
          ].filter(Boolean).map(row => (
            <div key={row!.label} className="flex gap-4 px-4 py-3">
              <span className="text-xs text-zinc-600 w-24 flex-shrink-0">{row!.label}</span>
