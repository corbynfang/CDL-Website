		...
		return (
			<div className="space-y-2 animate-pulse">
				{Array.from({ length: 10 }).map((_, i) => (
					<div
						key={i}
						className="h-12 rounded border border-zinc-800 bg-zinc-900/60"
					/>
				))}
			</div>
		);
	...
	if (!stats || stats.length === 0) {
		return (
			<p className="text-center text-zinc-600 py-16 text-sm">
				No stats available for this event.
			</p>
		);
	}
		...
		<div className="border border-[#1a1a1a] divide-y divide-[#1a1a1a]">
			...
			{stats.map((s, idx) => {
				...
				return (
					...
					>
						<span className="col-span-1 text-xs text-zinc-600 tabular-nums">
							{idx + 1}
						</span>
						...
						<span
							className={`col-span-2 text-right text-sm font-bold tabular-nums ${kdColor(s.kd_ratio)}`}
						>
