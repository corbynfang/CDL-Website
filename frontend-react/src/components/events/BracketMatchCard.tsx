...
export default function BracketMatchCard({ match }: Props) {
	...
	function TeamRow({
		...
		return (
			<div
				className={`relative flex items-center gap-2 pl-2.5 pr-3 py-2.5 ${dimmed ? "opacity-40" : ""}`}
			>
				{won && (
					<span className="absolute left-0 inset-y-0 w-0.5 bg-white rounded-full" />
				)}
				...
				<span
					className={`text-[12px] flex-1 truncate ${won ? "text-white font-bold tracking-wide" : "text-zinc-400"}`}
				>
					{name}
				</span>

				<span
					className={`text-[12px] font-mono font-bold tabular-nums min-w-[18px] text-right ${
