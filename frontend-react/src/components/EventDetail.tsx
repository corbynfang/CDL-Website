...
export default function EventDetail() {
  ...
  const {
    ...
    error: matchesError,
  } = useApi<Match[]>(`/api/v1/tournaments/${id}/matches`, {
  ...
  const {
    ...
    error: bracketError,
  } = useApi<BracketData>(`/api/v1/tournaments/${id}/bracket`, {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {tab === "teams" && <EventTeams teams={teams} loading={teamsLoading} />}
        {tab === "stats" && <EventStats stats={stats} loading={statsLoading} />}
      </div>
    </div>
  );
}
