...
export default function Events() {
  ...
  const visible = useMemo(() => {
    ...
    return events.filter((e) => {
      ...
      if (
        filters.status &&
        deriveStatus(e.start_date, e.end_date) !== filters.status
      )
        return false;
      return true;
    });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {liveEvents.length > 0 && (
        <div className="mb-6 space-y-2">
          {liveEvents.map((e) => (
            <LiveEventStrip key={e.id} event={e} />
          ))}
        </div>
      )}
      ...
      {!loading && visible.length === 0 && (
        <p className="text-center text-[#737373] py-20 text-sm">
      ...
      {!loading &&
        byMonth.map(([month, monthEvents]) => (
          <div key={month} className="mb-10">
            <p className="text-xs uppercase tracking-widest text-zinc-700 mb-4">
              {month}
            </p>
